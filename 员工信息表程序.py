...
def dict_file():    # 将文本输出为字典
    dict = {}
    dict_all = {}
    with open("员工信息存档", 'r', encoding='utf-8') as f:
        for line in f:
...
def file_dict(dict_all):    # 将字典输出到文本
    with open("员工信息存档",'w',encoding='utf-8') as f:
...
def employee_exit():    #定义退出功能
    ...
    while flag_exit:
        if b_input == 'b':
            ...
            return employee_exit()   #使用尾递归优化，加上return帮助直接退出，而不需要递归一层层退出
